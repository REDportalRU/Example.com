...
export default {
    ...
    setup(props) {
                ...
                onFinish: visit => {
                    ...
                    if (form.hasErrors) {
                        ...
                        Func.Notify(
                            ...
                            '<span style="font-size: .90rem">Страница "' + form.name + '" обновлена.</span>')
                    }
                },
            })
        }
        ...
        function deletePage() {
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.isConfirmed) {
                    // отправка формы
                    form.post('/page/delete/' + props.id, {
                        headers: {
                            'Content-Type': 'multipart/form-data'
                        },
                        // после получения ответа
                        onFinish: visit => {
                            ...
                            Func.Notify(
                                'success',
                                '<span style="font-size: 1.1rem">Успешно!</span>',
                                '<span style="font-size: .90rem">Страница "' + form.name + '" удалена.</span>');
                        },
                    })
                } else if (
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        text: "Страница '" + form.name + "' в безопасности :)",
                        icon: "error"
                    });
                }
            });
        ...
        return {form, updatePage, deletePage, getStatus}
    },
    layout: Layout
}
</script>
